...
export function ApplicationCard({ application, statusOptions }: ApplicationCardProps) {
  ...
  const statusAccentClass = STATUS_ACCENTS[application.status] ?? STATUS_ACCENTS.Applied;
  ...
  return (
    <>
      <li
        className={`rounded-3xl border border-t-4 bg-white p-5 shadow-[0_8px_24px_-14px_rgba(15,23,42,0.35)] transition-all duration-200 hover:-translate-y-1 hover:shadow-[0_14px_30px_-16px_rgba(15,23,42,0.45)] dark:bg-slate-900 ${
          isFollowUpOverdue
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex flex-wrap justify-end gap-2">
              <span
                className={`shrink-0 rounded-full px-3 py-1 text-[11px] font-semibold tracking-wide ${statusClass}`}
              >
                {application.status}
              </span>
              <span
                className={`shrink-0 rounded-full px-3 py-1 text-[11px] font-semibold tracking-wide ${priorityClass}`}
              >
